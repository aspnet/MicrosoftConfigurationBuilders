...
project, then click the 'X' on the tab to close and continue on.

However, if you are updating this package from a version earlier than 1.0.2... you may want to double check the
declarations for your config builders, because we might have lost any changes you made to the default declarations
...
updating will declare the default config builder with the default parameters again.

Starting in version 1.0.2, we have a way to stash old declarations aside and restore them upon install. So you should
not be inconvenienced like this in the future. Sorry for the trouble.
